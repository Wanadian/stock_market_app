...
*Attention : lors du premier lancement de l'application (dans la journée), veuillez prendre en compte un temps d'initialisation d'environ 2 à 3 minutes. Cela est nécessaire pour permettre à l'application d'effectuer l'appel à l'API (3 x 13 secondes, car nous sommes limités par l'API) et de remplir la base de données (pour nos trois entreprises, il doit y avoir environ 429 documents dans la collection "shares" de notre base de données Firestore Database). Vous avez également la possibilité d'ouvrir la base de données en temps réel via votre navigateur (le lien se trouve en bas de se readme) pour observer le processus de remplissage en direct. Cette courte période d'attente initiale garantit que l'application dispose des données les plus récentes. Il se peut que le message "Something went wrong" s'affiche à l'écran, cela veut donc dire que la base de données n'a pas eu le temps de se remplir complétement. Par conséquent, veuillez supprimer la collections "shares" de Firebase et relancer l'application, lors du relancement de cette dernière la base de données sera re-remplie (guardez en tête le temps d'initialisation).*
...
- <u>Consulter le total de nos actions et de notre portefeuille</u> : Sur l'écran d'accueil, le solde du portefeuille et des actions achetées est visible.
...
- <u>Gestion de l'attente des données API</u> : Lors du premier appel de la journée, il aurait été intéressant de mettre en place un widget d'attente, le temps que la base de données se vide et se remplisse (cela éviterait le message d'erreur évoqué précédemment).
